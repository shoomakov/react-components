import React, { Component, PropTypes } from 'react';
import importcss from 'importcss';
import classnames from 'classnames';

export default class AlphabeticAvatar extends Component {
  render() {
    return(
      <svg
        //  xmlns:dc="http://purl.org/dc/elements/1.1/"
        //  xmlns:cc="http://creativecommons.org/ns#"
        //  xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
        //  xmlns:svg="http://www.w3.org/2000/svg"
         xmlns="http://www.w3.org/2000/svg"
        //  xmlns:xlink="http://www.w3.org/1999/xlink"
        //  xml:space="preserve"
         viewBox="-27 24 100 100"
         version="1.1"
         id="unknown"
         enable-background="new -27 24 100 100">
         <defs id="defs23" />
        <g id="g3">
          <g id="g5">
            <rect
              id="rect7"
              y="24"
              x="-27"
              width="100"
              height="100"
              fill="#f2f2f2"
            />
            <g id="g9">
              <defs id="defs11">
                <path
                  id="shoulders"
                  d="M36,95.9c0,4,4.7,5.2,7.1,5.8c7.6,2,22.8,5.9,22.8,5.9c3.2,1.1,5.7,3.5,7.1,6.6v9.8H-27v-9.8
                  c1.3-3.1,3.9-5.5,7.1-6.6c0,0,15.2-3.9,22.8-5.9c2.4-0.6,7.1-1.8,7.1-5.8c0-4,0-10.9,0-10.9h26C36,85,36,91.9,36,95.9z"
                />
              </defs>
              <use
                id="use14"
                xlinkHref="#shoulders"
                overflow="visible"
                fill="#ccc"
              />
              <clipPath id="shoulders_1_">
                <use
                  id="use17"
                  xlinkHref="#shoulders"
                  overflow="visible"
                />
              </clipPath>
              <path
                id="head-shadow"
                fill="#787878"
                d="M23.2,35c0.1,0,0.1,0,0.2,0c0,0,0,0,0,0
                c3.3,0,8.2,0.2,11.4,2c3.3,1.9,7.3,5.6,8.5,12.1c2.4,13.7-2.1,35.4-6.3,42.4c-4,6.7-9.8,9.2-13.5,9.4c0,0-0.1,0-0.1,0
                c-0.1,0-0.1,0-0.2,0c-0.1,0-0.1,0-0.2,0c0,0-0.1,0-0.1,0c-3.7-0.2-9.5-2.7-13.5-9.4c-4.2-7-8.7-28.7-6.3-42.4
                c1.2-6.5,5.2-10.2,8.5-12.1c3.2-1.8,8.1-2,11.4-2c0,0,0,0,0,0C23.1,35,23.1,35,23.2,35L23.2,35z"
                clip-path="url(#shoulders_1_)"
              />
            </g>
          </g>
          <path
            id="head"
            fill="#ccc"
            d="M22.6,40c19.1,0,20.7,13.8,20.8,15.1c1.1,11.9-3,28.1-6.8,33.7c-4,5.9-9.8,8.1-13.5,8.3
            c-0.2,0-0.2,0-0.3,0c-0.1,0-0.1,0-0.2,0C18.8,96.8,13,94.6,9,88.7c-3.8-5.6-7.9-21.8-6.8-33.8C2.3,53.7,3.5,40,22.6,40z"
          />
        </g>
        <text
          id="text4172"
          y="100.22916"
          x="-20.563669"
          style={{
            'font-style': 'normal',
            'font-weight': 'normal',
            'font-size': '64.4928894px',
            'line-height': '125%',
            'font-family': 'sans-serif',
            'letter-spacing': '0px',
            'word-spacing': '0px',
            fill: '#000000',
            'fill-opacity': '1',
            stroke: 'none',
            'stroke-width': '1px',
            'stroke-linecap': 'butt',
            'stroke-linejoin': 'miter',
            'stroke-opacity': '1'
          }}
          xmlSpace="preserve">
          <tspan
            y="100.22916"
            x="-20.563669"
            id="tspan4174">SK
          </tspan>
        </text>
      </svg>
    )
  }
}
